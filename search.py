...
def graph_search(problem, fringe, branch=False, heuristic=False):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            ...
            return node, generated_nodes, expanded_nodes, format(end - start, ".1E")
        if node.state not in closed:
            ...
            if branch is True:
                fringe.sort(heuristic, problem)
    end = time.perf_counter() * 1000
    return None, generated_nodes, expanded_nodes, format(end - start, ".1E")
...
def branch_and_bound_graph_search(problem):
    return graph_search(problem, FIFOQueue(), True)


def branch_and_bound_heuristic_graph_search(problem):
    return graph_search(problem, FIFOQueue(), True, True)
